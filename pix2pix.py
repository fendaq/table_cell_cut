...
parser.add_argument("--max_num_input", type=int, default=-1, help="number of images in batch")
parser.add_argument("--lr", type=float, default=0.0002, help="initial learning rate for adam")
parser.add_argument("--beta1", type=float, default=0.5, help="momentum term of adam")
parser.add_argument("--l1_weight", type=float, default=100.0, help="weight on L1 term for generator gradient")
parser.add_argument("--gan_weight", type=float, default=1.0, help="weight on GAN term for generator gradient")
parser.add_argument("--max_loss_distance", type=float, default=0.5, help="weight on GAN term for generator gradient")
parser.add_argument("--crop_size", type=int, default=256, help="scale images to this size before cropping to 256x256")
...
def load_examples():
    ...
    input_paths = input_paths[:a.max_num_input]
...
def create_model(inputs, targets):
    ...
    with tf.name_scope("generator_train"):
        # cond = lambda: tf.greater_equal(tf.abs(gen_loss_GAN-discrim_loss), 0.5)
        ...
        if a.mode == "test":
            ...
            for step in range(max_steps):
                ...
                fetches = {
                    ...
                    'd_loss':model.discrim_loss,
                    'g_loss':model.gen_loss_GAN
                }
                g_train_count += 1
                
                if train_dis:
                    d_train_count += 1
                    fetches['dis_train'] = model.dis_train
                ...
                if (results['g_loss'] - results['d_loss'])  > a.max_loss_distance:
                    train_dis = False
                else:
                    train_dis = True
                ...
                if should(a.progress_freq):
                    ...
                    print('gen_dis_distance:', results['d_loss'] - results['g_loss'])
                    print('Train dis: ', train_dis)
                    print('Gtrain_count: {}\t D_train_count: {}'.format(g_train_count, d_train_count)) 
