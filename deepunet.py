...
Model = collections.namedtuple("Model", "outputs, gen_loss_L1, train")
...
def create_generator(generator_inputs, generator_outputs_channels):
    x = conv_bn_relu(generator_inputs, a.ngf//2)
    print(generator_inputs.shape)
    net = conv_bn_relu(x, a.ngf)
    bn1 = batchnorm(gen_conv(net, a.ngf//2))
    act1 = tf.nn.relu(bn1)

    bn2, act2 = down_block(act1)
    bn3, act3 = down_block(act2)
    bn4, act4 = down_block(act3)
    bn5, act5 = down_block(act4)
    bn6, act6 = down_block(act5)
    bn7, act7 = down_block(act6)

    print('Act7:{}\t, bn6:{}'.format(act7, act7))
...
def create_model(inputs, targets):
    with tf.variable_scope("generator"):
        ...
        print('DEPUNET:', a.deepunet)
        score1, score2, score3 = create_generator(inputs, out_channels)   
        outputs = score1
        h, w = inputs.get_shape().as_list()[1:3]
        outputs = tf.image.resize_nearest_neighbor(outputs, (h,w))
    with tf.name_scope("generator_loss"):
        def get_tagers(target_images):
            r = target_images
            r1 = tf.image.resize_images(target_images, [CROP_SIZE//2, CROP_SIZE//2], method=tf.image.ResizeMethod.AREA)
            r2 = tf.image.resize_images(target_images, [CROP_SIZE//4, CROP_SIZE//4], method=tf.image.ResizeMethod.AREA)
            return [r, r1, r2]
        targets1, targets2, targets3 = get_tagers(targets)


        # gen_loss_L1 = tf.reduce_mean(tf.square(targets3-score3))
        gen_loss_L1 = tf.reduce_mean(tf.square(targets1-score1)) + tf.reduce_mean(tf.square(targets2-score2)) + tf.reduce_mean(tf.square(targets3-score3))
    ...
    if a.mode == "export":
        ...
        with tf.variable_scope("generator"):
            batch_output = deprocess(
                create_generator(preprocess(batch_input_placeholder), 3)[-1])
