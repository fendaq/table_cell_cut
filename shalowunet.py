...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    bn4, act4 = down_block(act3, pool_size=4)
    bn5, act5 = down_block(act4, pool_size=4)

    print('Act6:{}\t, bn5:{}'.format(act5.shape, bn4.shape))
...
def create_model(inputs, targets):
    with tf.variable_scope("generator"):
        ...
        scores = create_generator(inputs, out_channels)   
    ...
    with tf.name_scope("generator_loss"):
        def get_tagers(target_images):
            ...
            for score in scores:
                ...
                rv.append(r)
            return rv
        targets = get_tagers(targets)


        # gen_loss_L1 = tf.reduce_mean(tf.square(targets3-score3))
        for t,s in zip(targets, scores):
            print(t.shape, s.shape)
        gen_loss_L1 = tf.reduce_mean([tf.reduce_mean(tf.square(t-s)) for t,s in zip(targets, scores)])
    ...
    if a.mode == "export":
        ...
        with tf.variable_scope("generator"):
            batch_output = deprocess(
                create_generator(preprocess(batch_input_placeholder), 3)[0])
        ...
        n1 = tf.identity(n1, name='h')
        n2 = tf.identity(n2, name='w')
